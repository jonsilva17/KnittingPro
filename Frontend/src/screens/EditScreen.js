...
export default function EditScreen({ route, navigation }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        console.log("Erro ao buscar padrões:", err);
      }
    })();
  }, []);
  ...
  const handleCreate = async () => {
    ...
    setLoading(true);
    try {
      // Criamos o objeto de opções explicitamente para evitar erros de referência
      const sweaterOptions = {
        colors: colors,
        size: size,
        gaugeStitches: gs,
        gaugeRows: gr,
      ...
      const result = await createSweaterPattern(imageUri, sweaterOptions);
      
      // Garante que o result existe antes de navegar
      if (result) {
        navigation.replace('Preview', { result, isSweater: true, isStitchBlanket: false });
      } else {
        Alert.alert('Erro', 'O servidor não devolveu dados válidos.');
      }
    } catch (e) {
      console.log("Erro capturado no botão:", e);
      Alert.alert('Erro na Criação', e.message || 'Ocorreu um erro ao gerar o padrão.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {/* BOTÃO PRINCIPAL CORRIGIDO COM ESTILO ISOLADO */}
      <TouchableOpacity
      ...
      >
        {loading ? (
          ...
          <Text style={styles.submitButtonText}>Gerar Padrão de Sweater</Text>
        )}
      </TouchableOpacity>
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  // ESTILOS EXCLUSIVOS DO BOTÃO GERAR TOTALMENTE PROTEGIDOS
  submitButton: {
