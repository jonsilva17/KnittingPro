...
export default function PreviewScreen({ route }) {
  const { result, isSweater, isStitchBlanket, isToy, isStitchEditor, isColorworkEditor } = route.params;
  ...
  const handleDownloadPdf = async (customName) => {
    ...
    if (Platform.OS === 'web') {
      ...
      a.href = url;
      a.download = customName || 'padrao_sweater.pdf';
    ...
    return (
      <ScrollWrapper style={styles.container} contentContainerStyle={styles.content}>
        ...
        <Text style={styles.dimensions}>
          {result.patterns_used?.length || 0} secções · {result.total_width} pts × {result.total_height} carreiras
        </Text>
  ...
  if (isToy) {
    const { chart, colors, toy_name, size, panel_width_cm, panel_height_cm } = result;
    return (
      <ScrollWrapper style={styles.container} contentContainerStyle={styles.content}>
        <Text style={styles.title}>🧸 {toy_name || 'Brinquedo'}</Text>
        <Text style={styles.dimensions}>
          Tamanho {size} · {panel_width_cm}cm × {panel_height_cm}cm
        </Text>
        <ChartGrid chart={chart} colors={colors || result.base_colors} cellSize={14} />
        <ColorKey colors={colors || result.base_colors} />
        <Text style={styles.sectionTitle}>Materiais</Text>
        <View style={styles.materialsBox}>
          <MaterialRow label="Cores de lã" value={colors?.length || result.base_colors?.length || 0} />
          <MaterialRow label="Lã estimada" value={`~${result.estimated_yarn_grams || 50}g`} />
          <MaterialRow label="Enchimento" value={`~${result.stuffing_grams || 25}g`} />
          <MaterialRow label="Agulhas" value="3.5mm - 4.5mm" />
        </View>
        <Text style={styles.sectionTitle}>Instruções Rápidas</Text>
        <View style={styles.materialsBox}>
          <Text style={styles.shapingText}>1. Tricote 2 painéis (frente + costas)</Text>
          <Text style={styles.shapingText}>2. Costure à volta (deixe abertura)</Text>
          <Text style={styles.shapingText}>3. Encha com fibra siliconada</Text>
          <Text style={styles.shapingText}>4. Feche a abertura</Text>
        </View>
        <TouchableOpacity
          style={[styles.downloadButton, downloading && styles.buttonDisabled]}
          onPress={() => handleDownloadPdf(`brinquedo_${toy_name || 'toy'}.pdf`)}
          disabled={downloading}
        >
          {downloading ? (
            <ActivityIndicator color="#FFFFFF" />
  ...
  if (isStitchEditor || isColorworkEditor) {
    const isColorwork = isColorworkEditor;
    const fileName = isColorwork ? 'padrao_jacquard.pdf' : 'padrao_pontos.pdf';
    return (
      <ScrollWrapper style={styles.container} contentContainerStyle={styles.content}>
        <Text style={styles.title}>{isColorwork ? 'Padrão Jacquard' : 'Padrão de Pontos'}</Text>
        <Text style={styles.dimensions}>
          {result.width || 0} pts × {result.height || 0} carreiras
        </Text>
        <TouchableOpacity
          style={[styles.downloadButton, downloading && styles.buttonDisabled]}
          onPress={() => handleDownloadPdf(fileName)}
  ...
  if (!isSweater) {
    ...
    return (
      <ScrollWrapper style={styles.container} contentContainerStyle={styles.content}>
        ...
        <TouchableOpacity
          style={[styles.downloadButton, downloading && styles.buttonDisabled]}
          onPress={() => handleDownloadPdf('padrao_chart.pdf')}
          disabled={downloading}
        >
          {downloading ? (
            <ActivityIndicator color="#FFFFFF" />
